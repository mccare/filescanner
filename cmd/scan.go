...
func insertNewFileWithMD5(file File) File {
	file = insertFileIfNew(file)
	file = updateMd5(file)
	return file
}

// scan folder is synchronous, will return after all workers have finished
func scanFolder(processor Processor, maxProcessors int) {
...
// TODO information retrieved is not copied into the file object
...
func NewScanCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if Checkdb {
				scanDB(checkAndMarkDeletedFile, "files", 100)
				return
			}
			if ScanTags {
				fmt.Printf("Scnaning tags")
				scanDB(scanTagsAndUpdateDB, "music_files", 10)
				return
			}
			scanFolder(insertNewFileWithMD5, 10)
		},
	}
